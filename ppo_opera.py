...
from catdogmodel import CatAndDogConvNet, CatAndDogConvNetCustom
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__{args.exp_name}__{args.seed}__{args.use_custom_reward}__env_rewards_{args.use_env_reward}__{args.num_steps}__{args.total_timesteps}__{int(time.time())}"
    ...
    model_path = os.path.join(reward_models_dir, f"reward_model_curated_prefs.pth")
    h, w = 224, 224
    if 'rlvlmf' in model_path:
        reward_model = gen_image_net(image_height=h, image_width=w)
    else:
        #reward_model = CatAndDogConvNet()
        reward_model = CatAndDogConvNetCustom(400, 600)
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if args.use_custom_reward and args.use_env_reward:
                  ...
                  custom_reward = torch.nn.Sigmoid()(reward_model(img.unsqueeze(0))).view(-1).numpy()
                else:
                  custom_reward = [-0] 
    ...
    with open(f'./summary/curated_{args.env_id}_custom_{args.use_custom_reward}_{args.total_timesteps}_env_rewards_{args.use_env_reward}.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['env_reward', 'custom_reward'])
    ...
    plt.legend()
    plt.savefig(f"rewards_curated_custom_reward_{args.use_custom_reward}_env_rewards_{args.use_env_reward}_{args.total_timesteps}_{args.num_steps}_ppolr_{args.learning_rate}.png")

    # save model
    os.system('mkdir -p trained_policies')
    torch.save(agent.state_dict(), f"./trained_policies/Agent_curated_{args.env_id}_{args.use_custom_reward}_env_rewards_{args.use_env_reward}_{args.total_timesteps}_ppolr_{args.learning_rate}_{int(time.time())}.pth")